...
function formulaires_editer_affectations_charger($id_plugin, $typologie, $options = array()) {
	...
	// On en déduit :
	// -- le nombre déjà affectés
	// -- l'indication d'atteinte ou pas du nombre maximal d'affectations.
	...
	$max_affectations_atteint = 0;
	if ($configuration_typologie['max_affectations']
	and ($nb_affectations == $configuration_typologie['max_affectations'])) {
		$max_affectations_atteint = 1;
	}
	...
	$valeurs = array(
		...
		'typologie_complete' => $max_affectations_atteint,
		'attribut_id'        => "plugin-${typologie}",
		'_objet_lien'        => 'type_plugin',
