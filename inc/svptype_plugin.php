 ...
 *        Typologie que l'on souhaite renvoyer pour le plugin concernée : categorie ou tag.
 *
 * @return string|array
...
function plugin_lire_type($prefixe, $typologie) {
	...
	if (!isset($types[$typologie][$prefixe])) {
		...
		$configuration = lire_config("svptype/typologies/${typologie}", array());

		// Initialisation des affectations pour le plugin.
		$types[$typologie][$prefixe] = $configuration['max_affectations'] == 1 ? '' : array();
		...
		$where = array(
			...
			'spip_plugins_typologies.id_groupe=' . intval($configuration['id_groupe']),
